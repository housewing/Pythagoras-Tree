...
void draw(std::vector<Point> _points, int _level)
{
	...
	std::vector<Point> first = applyTransform(_points, scaleMatrix, _points[0], transPt, length);

	transPt = Point2f(1.0 / 2.0, 0.0);
	std::vector<Point> second = applyTransform(_points, scaleMatrix, _points[0], transPt, length);

	transPt = Point2f(1.0 / 4.0, std::sqrt(3) / 4.0);
	std::vector<Point> third = applyTransform(_points, scaleMatrix, _points[0], transPt, length);

	const Point* f[1] = { &first[0] };
	const Point* s[1] = { &second[0] };
	const Point* t[1] = { &third[0] };
	const Point* b[1] = { &_points[0] };
	int npt[] = { 3 };
